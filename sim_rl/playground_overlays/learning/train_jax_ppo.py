...
# Persistent XLA compile cache (same pattern as playground learning notebooks).
# Cold starts are CPU-bound on ptxas with GPU util near 0%; warm starts reuse
# this cache and reach sustained GPU compute much sooner.
...
def main(argv):
  ...
  if _LOAD_CHECKPOINT_PATH.value is not None:
    ...
    if ckpt_path.is_dir():
      # Parent dir (e.g. .../checkpoints) has numeric step children; a step
      # dir itself (e.g. .../checkpoints/000033423360) has Orbax subdirs like
      # "d" that are not step numbers.
      step_ckpts = [
          ckpt
      ...
      if step_ckpts:
        step_ckpts.sort(key=lambda x: int(x.name))
        restore_checkpoint_path = step_ckpts[-1]
      else:
  ...
  print(
      "Starting PPO train. Cold XLA compile is CPU-bound (ptxas); GPU util "
      "often stays near 0% until the first metrics line. Cache: "
      f"{_JAX_CACHE_DIR}"
  )
  ...
  # Render and save rollouts under the checkpoint step folder when possible
  # (e.g. .../checkpoints/000033423360/rollout0.mp4 for --play_only).
  video_dir = epath.Path(".")
  if restore_checkpoint_path is not None:
    video_dir = epath.Path(restore_checkpoint_path)
    if video_dir.is_file():
      video_dir = video_dir.parent
  else:
    step_ckpts = [
        c for c in ckpt_path.glob("*") if c.is_dir() and c.name.isdigit()
    ]
    if step_ckpts:
      step_ckpts.sort(key=lambda x: int(x.name))
      video_dir = step_ckpts[-1]
  ...
  for i, rollout in enumerate(trajectories):
    ...
    video_path = video_dir / f"rollout{i}.mp4"
